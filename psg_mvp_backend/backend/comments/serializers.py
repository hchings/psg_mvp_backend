...
class CommentReplySerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Comment
        fields = ('uuid', 'posted', 'author', 'like_num', 'liked_by_user',
                  'text', 'is_op')

    def __init__(self, *args, **kwargs):
        super(CommentReplySerializer, self).__init__(*args, **kwargs)
...
class CommentSerializer(serializers.ModelSerializer):
    ...
    replies_num = serializers.SerializerMethodField(required=False)
    replies = serializers.SerializerMethodField(required=False)

    class Meta:
        model = Comment
        fields = ('case_id', 'uuid', 'posted', 'author', 'like_num', 'liked_by_user',
                  'text', 'is_op', 'replies_num', 'replies')
    ...
    def __init__(self, *args, **kwargs):
        ...
        try:
            if self.context['request'].method in ['GET']:
                ...
                # POST?
                self.fields['reply_to_id'] = serializers.CharField(required=False)
                self.fields['author'] = AuthorSerializer()
        except KeyError:
            pass
    ...
    def get_replies_num(self, obj):
        return Comment.objects.filter(reply_to_id=obj.uuid).order_by('-posted').count()

    def get_replies(self, obj):
        return []
        # serializer = CommentReplySerializer(Comment.objects.filter(reply_to_id=obj.uuid, is_op=True).order_by('-posted'),
        #                                     many=True,
